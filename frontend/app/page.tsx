...
export default function Home() {
  ...
  const stablediffusionapi = (text : string) => {
    return fetch('/api/stablediffusion', {
      method: 'POST',
      body: text
    })
    ...
    .then(data => {
      console.log(data)
      document.getElementById("output").innerHTML = document.getElementById("output")?.innerHTML.replace("|||" + text + "|||", `<center><img width="400" src="${data}" /></center>`)
    })
